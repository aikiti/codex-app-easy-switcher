...
初期状態ではOllamaモデルを自動選択しません。インストール済みモデルを再取得し、
Codex対応可否を確認してからCloudまたはローカルモデルを選択します。
...
- [Mac版（最新v0.4.0）を直接ダウンロード](https://github.com/aikiti/codex-app-easy-switcher/releases/download/v0.4.0/Codex-App-Easy-Switcher-macOS.zip)
- [Windows版EXE（最新v0.4.0）を直接ダウンロード](https://github.com/aikiti/codex-app-easy-switcher/releases/download/v0.4.0/Codex-App-Easy-Switcher-Windows.exe)

MacではZIPを展開してアプリを起動します。WindowsではEXEをダブルクリックします。
v0.4.0では新しいCodex Appが配るレガシーな`profile`設定行を切り替え直後に取り除くようになり、
「failed to resolve feature override precedence」のエラーが解消されています。
...
- Cloud・ローカルモデルをタブで分けた初心者向け選択画面
- `thinking`・`tools`・64K以上のコンテキストを使ったCodex互換性確認
...
`CODEX_HOME`が設定されている場合はその配下、未設定なら`~/.codex/config.toml`を
読み取ります。切り替え直前に`CodexModelLauncher/backups`へアプリ独自の日時付き
バックアップを作成し、設定変更・通常状態への復元はOllama Launchへ委譲します。
新しいCodexが拒否するOllama Launch由来のトップレベル`profile`行だけは、
切り替え後に一時ファイルと`os.replace`を使って除去します。

### Ollamaモデル管理

- インストール済みモデルをCloud／ローカルのタブに分けて一覧表示
- Cloudモデルとローカルモデルを区別
- Ollamaを「未インストール／停止中／起動中」で表示
- モデル操作時にOllamaが停止中なら確認後に起動し、最大15秒で接続判定
- 勉強会向け6モデルをメモリ目安・サイズ・画像対応・Codex用途付きで表示
- 各モデルのOllama公式ページを開き、公式コマンドをコピー可能
- `gemma3:1b`はOllama体験専用。Codex用には自動選択しない
- Codex対応モデルは取得後にCodex App用モデルとして自動選択
- 削除済み・Codex非対応モデルが選択中の場合は、一覧取得成功後に選択を解除
- 手入力したモデルを確認後にインストール
- Ollama公式ページの`ollama run <モデル名>`を貼り付けた場合はモデル名だけを安全に抽出
- インストール進捗表示と中止
- 大容量通信・ディスク使用の可能性を開始前に警告
- Ollama公式ダウンロードページを常時表示
...
2. 「Ollama モデル管理」タブの「Ollama公式ダウンロード」を押します。
3. 開いたOllama公式サイトからWindows版をダウンロードし、インストーラーを実行します。
4. 本アプリへ戻り、「状態を再確認」を押して「Ollama: 起動中（バージョン）」を確認します。
5. PCメモリに合うモデルカードを選び、確認画面からダウンロードします。

本アプリは安全のためOllamaのインストーラーを自動実行せず、公式ダウンロードページだけを
開きます。Ollamaがインストール済みでも停止中の場合は、モデル操作時に確認画面を表示し、
了承後にOllamaを起動して最大15秒で接続を確認します。この一連のWindows実機動作は
2026年8月20日に確認済みです。

### Codex App用モデルを変更する

1. 「モデルを変更」を押します。
2. アプリがOllamaからモデル一覧と能力を再取得するまで待ちます。
3. Cloudまたは「インストール済みローカルモデル」タブから選びます。
4. Codex対応モデルを選択し、メイン画面で起動ボタンを押します。

選択したモデルが未準備の場合、起動せず「Ollama モデル管理」タブへ案内します。
Codex対応モデルの選択は次回起動時も保存されるため、毎回選び直す必要はありません。
削除済みまたはCodex非対応になったモデルは、一覧取得成功後に選択から外れます。
...
勉強会では「Ollama モデル管理」タブの6モデルから、PCメモリに合うカードを選びます。
メモリ表記はGPU・コンテキスト設定で変動する目安です。`gemma3:1b`はOllama体験専用、
その他は取得後に実際の能力を確認してCodex対応なら選択できます。
`gemma3:1b`だけをインストールした場合はCodex用に自動選択されず、
「Codex対応モデルがありません」と表示されるのが正常です。

その他のモデルを使う場合は、
「Ollama モデル管理」タブでOllama公式ライブラリのモデル名を入力し、
「モデルをインストール」を押します。公式ページの`ollama run <モデル名>`コマンド全体を
貼り付けても構いません。

## 安全設計

- Cloud／ローカルモデルを分離表示し、選択時・起動時に互換性確認と警告
- モデル変更だけではCodex Appを終了・再起動しない
- subprocessは引数リスト形式で実行し、`shell=True`を使用しない
- 危険な文字を含むモデル名を拒否
- 切り替え直前にCodex設定をアプリ独自に自動バックアップ

...
- Ollama未導入状態から公式ダウンロードページを経由する初心者向けセットアップ
...
- Codex対応モデルの選択はアプリ設定へ保存し、次回起動時も維持する。
...
- Ollama未導入PCで、公式ページを開く→Ollamaをインストール→状態再確認→モデル取得ができる。
- Ollama停止中にモデル操作すると、確認後に起動し15秒以内で接続結果が表示される。
- `gemma3:1b`だけがある場合はCodex用に選択せず、Codex対応モデルがないことを案内する。
- モデルが1～2件の環境でモデル再取得が無期限に待機しない。
